...
a deduction rule of the form \code{instance Arbitrary b, Arbitrary
  (a -> b) => Arbitrary b where ...}; but Haskell's type-class
...
a signature with two abstract types \code{t, u} and a single operation
of type \code{(t -> t) -> u}: the only way to produce a \code{u} is to
...
\code{Articheck} amenable to change in the generation technique; we

...
\begin{itemize}
  ...
  \item If some function takes, say, a tuple, the code as it stands will construct
    the set of all possible tuples, perform a \code{map}, then finally call
    \code{destruct} on each resulting element to collect instances.
  ...
  \item Take the example of logical formulas. The naïve strategy consists in
    ...
    by, say, \code{mk\_and} to \code{mk\_xor} so as to generate formulas with
...
Consider the case of a function that has type \code{t -> t -> t}. The outer call
...
\code{apply}. That way, we save a copy of the arguments that are to be applied
in each \code{Fun} case.
...
our \code{'a ty} (type descriptor) and properties to be \code{'a lists} (the
